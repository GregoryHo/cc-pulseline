...
fn renders_core_metrics_from_stdin_and_local_sources() {
    ...
    // L3 context cell now renders as `<used>/<total>` — no `CTX:` label,
    // no `%` (the gauge mode adds a visual ratio; flat-row text mode
    // shows only the concrete numbers).
    assert_eq!(
        lines[2],
        "86.0k/200.0k | TOK I:10 O:20 C:30/40 | $3.50 ($3.50/h)"
    );
}
