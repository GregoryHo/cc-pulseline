...
- **`state/mod.rs`** — `SessionState` holds per-session mutable state: transcript file offset, active tools/agents/todo lists, recent tools (persist after completion for display), cached env/git snapshots, and the cache-trend buffers (`cache_history` hit-rate FIFO capped at 30 samples + cumulative `cache_read_total` deduped per API call by `message.id`; the total survives `compact_boundary`, both clear on transcript path change/truncation). `PulseLineRunner` maintains a `HashMap<String, SessionState>` keyed by session+transcript+project.
...
- **`render/`** — Pure rendering logic, split into submodules:
  ...
  - `pane.rs` — `LayoutStyle` enum (`None`/`Compact`/`Budgets`/`Console`/`Ledger`; treat the enum as the source of truth) + `PaneConfig` chrome wrapper. `apply_pane()` decorates the assembled lines with frame chrome.
  - `frames/` — one `render()` fn per pipeline-owning layout (`console`, `ledger`, `rail`, `anchor`; `powerline.rs` is rail's seam helper) — see `src/render/frames/`. `console` renders the single outer `╭─...─╮` frame with `├─┼─┤` group separators and the Identity row hoisted into the top frame title. `frames/shared.rs` holds the box-drawing glyphs, label/content padding, identity headline, config row, and the per-segment dispatch hubs (`render_context_visual` and `render_quota_visual` — each maps a `+`-joined visual spec onto `widgets::gauge::render` / `widgets::sparkline::render` / inline text cells). `frames/mod.rs::default_visuals_for(LayoutStyle)` is the per-layout `*_visual` defaults table.
  - `widgets/` — Atomic widget renderers: `gauge` (bracketless marks-on-track — `▰` filled / `─` empty / `·` threshold marks in Icon mode, `=` / `-` / `:` in Ascii), `sparkline` (braille, icon-only — caller picks the fill color), `plot` (icon-only normalized braille line plot), and `effort` (ordinal pip-ramp, NOT icon-gated — renders under Ascii/NO_COLOR via `=`/`-` fallback). Signatures differ per widget — see `widgets/*.rs`; ascii-incompatible widgets return `""` so dispatch hubs drop them cleanly. `gauge`'s `width` is the visible cell count (no frame); caller supplies threshold marks (CTX → `ThemePalette::ctx_marks()` = `[55, 70]`; quota → `[50, 85]`).
