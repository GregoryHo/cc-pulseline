...
- **Cumulative cache read total**: honest raw sum of `cache_read_input_tokens` from transcript `message.usage` events, deduped per API call on two axes — by `message.id` (streaming chunks of one call count once) and by a timestamp high-water mark (session resume re-appends the full history into the same transcript file with original timestamps; replayed events at or below the mark are skipped). NOT cleared by `compact_boundary` — session-cumulative, survives compaction. Cleared on transcript path change.
